defmodule OfficeServerWeb.CoreComponents do
  @moduledoc """
    ...
    <div class="mt-14">
      <dl class="-my-4 divide-y divide-zinc-100">
        <div :for={item <- @item} class="flex gap-4 py-4 text-sm leading-6 sm:gap-8">
          ...
          <dd class="text-zinc-700" data-title={item.title}><%= render_slot(item) %></dd>
        </div>
      </dl>
    </div>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(ScratchyWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(ScratchyWeb.Gettext, "errors", msg, opts)
    end
  end
